...
# Open Redirect — Theory & Deep Technical Analysis
...
Occurs within the context of url parameter redirect, historically bypassing generic validation defenses due to its unique rendering or execution environment.

**Vulnerable code:**
```javascript
// Input saved directly, rendered without encoding or parameterization
processInput('URL Parameter Redirect_context', user_input);
```

**Fixed code:**
```javascript
// Safely parameterized or sanitized using contextual filters
processInput('URL Parameter Redirect_context', escapeAndSanitize(user_input));
```

### Login/Logout Redirect
Occurs within the context of login/logout redirect, historically bypassing generic validation defenses due to its unique rendering or execution environment.

**Vulnerable code:**
```javascript
// Input saved directly, rendered without encoding or parameterization
processInput('Login/Logout Redirect_context', user_input);
```

**Fixed code:**
```javascript
// Safely parameterized or sanitized using contextual filters
processInput('Login/Logout Redirect_context', escapeAndSanitize(user_input));
```

### Header-based Redirect
Occurs within the context of header-based redirect, historically bypassing generic validation defenses due to its unique rendering or execution environment.

**Vulnerable code:**
```javascript
// Input saved directly, rendered without encoding or parameterization
processInput('Header-based Redirect_context', user_input);
```

**Fixed code:**
```javascript
// Safely parameterized or sanitized using contextual filters
processInput('Header-based Redirect_context', escapeAndSanitize(user_input));
```

### OAuth Redirect
Occurs within the context of oauth redirect, historically bypassing generic validation defenses due to its unique rendering or execution environment.

**Vulnerable code:**
```javascript
// Input saved directly, rendered without encoding or parameterization
processInput('OAuth Redirect_context', user_input);
```

**Fixed code:**
```javascript
// Safely parameterized or sanitized using contextual filters
processInput('OAuth Redirect_context', escapeAndSanitize(user_input));
```

## Root Cause Analysis

### 1. Context Confusion
Developers often sanitize for one context (e.g., HTML text) but safely render in another (e.g., JavaScript attribute or DOM sink). This failure to understand context switching is the leading root cause. Over 138 reports in this corpus involve some form of context confusion.
