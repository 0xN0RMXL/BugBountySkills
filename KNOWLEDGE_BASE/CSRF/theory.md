...
# CSRF — Theory & Deep Technical Analysis
...
### State-Changing CSRF
Occurs within the context of state-changing csrf, historically bypassing generic validation defenses due to its unique rendering or execution environment.

**Vulnerable code:**
```javascript
// Input saved directly, rendered without encoding or parameterization
processInput('State-Changing CSRF_context', user_input);
```

**Fixed code:**
```javascript
// Safely parameterized or sanitized using contextual filters
processInput('State-Changing CSRF_context', escapeAndSanitize(user_input));
```

### Login CSRF
Occurs within the context of login csrf, historically bypassing generic validation defenses due to its unique rendering or execution environment.

**Vulnerable code:**
```javascript
// Input saved directly, rendered without encoding or parameterization
processInput('Login CSRF_context', user_input);
```

**Fixed code:**
```javascript
// Safely parameterized or sanitized using contextual filters
processInput('Login CSRF_context', escapeAndSanitize(user_input));
```

### JSON CSRF
Occurs within the context of json csrf, historically bypassing generic validation defenses due to its unique rendering or execution environment.

**Vulnerable code:**
```javascript
// Input saved directly, rendered without encoding or parameterization
processInput('JSON CSRF_context', user_input);
```

**Fixed code:**
```javascript
// Safely parameterized or sanitized using contextual filters
processInput('JSON CSRF_context', escapeAndSanitize(user_input));
```

### CSRF Token Bypass
Occurs within the context of csrf token bypass, historically bypassing generic validation defenses due to its unique rendering or execution environment.

**Vulnerable code:**
```javascript
// Input saved directly, rendered without encoding or parameterization
processInput('CSRF Token Bypass_context', user_input);
```
...
### 1. Context Confusion
Developers often sanitize for one context (e.g., HTML text) but safely render in another (e.g., JavaScript attribute or DOM sink). This failure to understand context switching is the leading root cause. Over 202 reports in this corpus involve some form of context confusion.
